...
router.route('/')
    .get(authenticateInterface, async (req: InterfaceRequest, res: Response, next: NextFunction) => {
        try {
            ...
            const menuInfo = await restaurant.getMenus()

            const menus = await Promise.all(menuInfo.map(menu => menu.getFullMenu()))


            const tabs = await Tab.getFullTabs(restaurant.id as string);

            return res.status(200).json({ tabs, menus });
        } catch (err) {
            next(err);
        };
    })
