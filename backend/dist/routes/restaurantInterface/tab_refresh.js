...
router.route('/')
    ...
    try {
        ...
        ;
        const menuInfo = yield restaurant.getMenus();
        const menus = yield Promise.all(menuInfo.map(menu => menu.getFullMenu()));
        const tabs = yield Tab_1.Tab.getFullTabs(restaurant.id);
