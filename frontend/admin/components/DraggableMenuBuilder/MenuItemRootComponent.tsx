...
const MenuItemRootComponent: React.FC<MenuItemRootComponentProps> = ({ itemRoot, jwt, menuId, sectionId, updateMenu }) => {
    ...
    return (
        <div className="mb-4 p-4 border rounded min-h-[150px] relative" key={itemRoot.position}>
            ...
            <div className="mt-2">
                {itemRoot.MenuItemVariations.map((variation, index) => (
                    <Draggable draggableId={variation.id} index={index} key={variation.id}>
                        {(provided) => (
                            <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                                <MenuItemVariationComponent key={variation.id} variation={variation} handleDeleteVariation={() => { handleDeleteVariation(variation) }} handleEditVariation={handleEditVariation} />
                            </div>
                        )}
                    </Draggable>
                ))}
