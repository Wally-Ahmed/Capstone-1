...
const MenuSectionComponent: React.FC<MenuSectionComponentProps> = ({ menuId, section, jwt, updateMenu }) => {
    ...
    return (
        <Droppable droppableId={section.id} type="item">
            {(provided) => (
                <div
                    className="mb-8 p-4 border rounded shadow-md min-h-[200px] relative pb-16"
                ...
                >
                    ...
                    {section.MenuItemRoots.map((itemRoot, index) => (
                        <Draggable draggableId={itemRoot.id} index={index} key={itemRoot.id}>
                            {(provided) => (
